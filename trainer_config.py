...
output.append(time_1_output_layer)
...
for i in range(1, TERM_SIZE):
    ...
    # former_input = np.mat(former_output)

    label = data_layer(name=key, size=4)



    recent_layer = fc_layer(input=former_output, size=TERM_SIZE, act=ReluActivation())

    fc_nn_layer = fc_layer(input=[lastseq_1_layer, recent_layer], act=ReluActivation(), size=TERM_SIZE*2)
